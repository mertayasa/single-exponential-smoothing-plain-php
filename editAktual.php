<?php
    ...
    $id = $_GET['id_aktual'];
    $aktual = mysqli_query($conn, "SELECT a.nama_barang, c.data_aktual, c.id_aktual, c.bulan, c.tahun 
                                        FROM tb_barang a, tb_aktual c 
                                        WHERE a.id_barang = $id_barang AND c.id_aktual = $id");
    $row = mysqli_fetch_array($aktual);

    if(isset($_POST["submit"])){
        $edit = editAktual($_POST);
        if(editAktual($_POST) > 0){
            echo "<script>
                    alert('data berhasil diubah!');
                    document.location.href = 'aktual.php?id_barang=". $_GET['id_barang'] ."';    
            </script>";
        } else {
            echo "<script>
                    alert('data gagal diubah!');
                    document.location.href = 'editAktual.php?id_aktual=$id';    
            </script>";
        }
    }
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <h6 class="card-subtitle">Slahkan Edit Data Aktual Barang!</h6>

                                <form action="" method="post">
                                    <input type="hidden" name="id_aktual" value="<?= $row['id_aktual']; ?>">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-6">
                                            <input type="text" class="form-control" id="data_aktual" name="data_aktual" value="<?= $row['data_aktual']; ?>" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-6">
                                            ...
                                            <a href="aktual.php?id_barang=<?=$id_barang?>" role="button" class="btn btn-danger">Kembali</a>
                                        </div>
                                    </div>
                                </form>
